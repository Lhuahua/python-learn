...
class Net():
    def __init__(self):
        ...
        self.hidden_num3 = 4
        self.output_num = 2
        self.W1 = np.random.rand(2, self.hidden_num1)
        self.b1 = np.zeros((1, self.hidden_num1))
        self.W2 = np.random.rand(self.hidden_num1, self.hidden_num2)
        self.b2 = np.zeros((1, self.hidden_num2))
        self.W3 = np.random.rand(self.hidden_num2, self.hidden_num3)
        self.b3 = np.zeros((1, self.hidden_num3))
        self.W4 = np.random.rand(self.hidden_num3, self.output_num)
        self.b4 = np.zeros((1, self.output_num))
    ...
    def forward(self, X):
        ...
        self.z4 = self.h3.dot(self.W3) + self.b3
    ...
    def grad(self, Y):
        ...
        """
        grad_z2 = self.y - Y
        self.grad_W2 = self.h3.T.dot(grad_z2)
        self.grad_b2 = np.sum(grad_z2, axis=0, keepdims=True)
        grad_h3 = grad_z2.dot(self.W2.T)

        grad_z1 = grad_h3 * self.h * (1-self.h)
